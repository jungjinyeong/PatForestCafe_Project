...
3. `BeginBreadUnavailableFlow()`(1.5~3초 대기) → `ShowSweatThenExit()`(`LobbyCharUI.SetSweatIconActive(true)`, 1.2초 대기) → `ExitEarlyDueToNoBread()`(아이콘 끄기 + `mBehaviorQueue.Clear()` + `ResumeFromPause()`). 큐를 비운 채 기존 `AdvanceBehaviorQueue()`를 그대로 호출하는 것만으로 "남은 큐(예: 음료 주문) 포기하고 곧장 Exit 웨이포인트로 향함 → `TryMoveToNextGroup()`으로 테라스 이동/디스폰"까지 전부 기존 로직 재사용.
